...
BOOKS_DIR=$DATA/ebook
#mkdir -p ~/Books

# Save find result to F_ARRAY
readarray -t F_ARRAY <<< "$(find "$BOOKS_DIR" -type f  -regex  ".*.epub\|.*.pdf" )"
...
main() {
  ...
  if [ -n "$book" ]; then
    #xdg-open "${BOOKS[$book]}"    # uncomment this line if you want to use sytems pdf reader
    zathura "${BOOKS[$book]}"
